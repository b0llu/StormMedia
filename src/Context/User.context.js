...
import { ADD_FOLLOWING, REMOVE_FOLLOWING, GET_USERS } from "Utils/Action";
...
const UserProvider = ({ children }) => {
  ...
  const followUser = async (id) => {
    try {
      ...
      if (response.status === 200) {
        dispatch({type: ADD_FOLLOWING, payload: response.data.user.following})
  ...
  return (
    <UserContext.Provider value={{ followUser, unfollowUser }}>
      {children}
